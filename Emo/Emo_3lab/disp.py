...
def calculate_variance_and_std_dev(csv_file, column_name):
    ...
    if len(best_f_values) > 1:
        ...
        return variance, std_dev,mean
    else:
        return None, None

csv_file = 'genetic_algorithm_results.csv'
column_name = 'Best f(x)'

variance, std_dev = calculate_variance_and_std_dev(csv_file, column_name)

if variance is not None:
    print(f'Выборочная дисперсия (Грея) "{column_name}": {variance}')
    print(f'Стандартное отклонение (Грея) "{column_name}": {std_dev}')
